-- Email Outreach Tables — idempotent migration
-- Run against: INTERNAL workspace DB
-- Created: 2026-03-10
-- This file is safe to re-run (IF NOT EXISTS everywhere).
...
CREATE TABLE IF NOT EXISTS email_campaigns (
  ...
  sent_at                timestamptz,
  -- Aggregate stats (updated by webhook handler via increment_campaign_stat RPC)
...
CREATE TABLE IF NOT EXISTS email_sends (
  ...
  contact_id            uuid,           -- alumni_contacts.id
  email                 text NOT NULL,
  first_name            text,
  last_name             text,
  grad_year             int,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'email_campaigns' AND policyname = 'ec_all'
  ) THEN
    CREATE POLICY "ec_all" ON email_campaigns FOR ALL USING (true) WITH CHECK (true);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'email_sends' AND policyname = 'es_all'
  ) THEN
    CREATE POLICY "es_all" ON email_sends FOR ALL USING (true) WITH CHECK (true);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'email_unsubscribes' AND policyname = 'eu_all'
  ) THEN
...
-- Uses CREATE OR REPLACE so it's always safe to re-run.
