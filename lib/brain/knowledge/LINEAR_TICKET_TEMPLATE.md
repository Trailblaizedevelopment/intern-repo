...
> Vendored for Dynamo (intern-repo) from greekspeed `docs/users/LINEAR_TICKET_TEMPLATE.md`.
> Do not depend on a local greekspeed path at runtime. Refresh from source when the template changes.

Use this structure when creating issues in Linear—for Cursor agents, OpenClaw, or humans. Clear tickets ship faster.

---

## Trailblaize / Cursor automation (required context)

| Rule | Value |
|------|--------|
| **GitHub repo** | Infer from the ask: `Trailblaizedevelopment/Trailblaize-Web` (default), `Trailblaizedevelopment/greekspeed`, or `owentrailblaize/intern-repo` (Dynamo / Brain / CRM). |
| **Base branch** | Product repos: `develop` — Cursor branches **from** `develop`, never from `main`. Dynamo (`intern-repo`): branch from `main`, PR into `main`. |
| **Tasks MVP override** | For Flagship Tasks program: branch from **`feature/task-mvp`**, PR into **`feature/task-mvp`**. |
| **Branch prefix** | `cursor/` + Linear id + slug, e.g. `cursor/TRA-158-add-edit-posts` |
| **Pull requests** | Product: open PR **into `develop` only**. Dynamo: PR into `main`. |
| **Deploy** | Product: `develop` → staging; `main` → production — human release flow. |

OpenClaw / Dynamo should **not** open PRs or push; they create/refine **Linear** issues. **Cursor Cloud Agent** implements after assignment.
...
*Source: greekspeed docs/users/LINEAR_TICKET_TEMPLATE.md (vendored for Dynamo).*
