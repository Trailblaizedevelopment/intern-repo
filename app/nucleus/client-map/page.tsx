...
> = {
  'university of tennessee': { lat: 35.9544, lng: -83.9282, state: 'TN', region: 'Southeast' },
  'university of mississippi': { lat: 34.3654, lng: -89.5373, state: 'MS', region: 'Southeast' },
  'texas a&m university': { lat: 30.6187, lng: -96.3365, state: 'TX', region: 'Southwest' },
  'texas a&m': { lat: 30.6187, lng: -96.3365, state: 'TX', region: 'Southwest' },
  'texas tech': { lat: 33.5843, lng: -101.8749, state: 'TX', region: 'Southwest' },
  'texas tech university': { lat: 33.5843, lng: -101.8749, state: 'TX', region: 'Southwest' },
  'university of alabama': { lat: 33.2148, lng: -87.5394, state: 'AL', region: 'Southeast' },
  'university of colorado boulder': { lat: 40.0076, lng: -105.2659, state: 'CO', region: 'West' },
  'university of colorado': { lat: 40.0076, lng: -105.2659, state: 'CO', region: 'West' },
  'university': { lat: 34.3654, lng: -89.5373, state: 'MS', region: 'Southeast' }, // fallback for bare "University"
  // Common extras
  'university of georgia': { lat: 33.9480, lng: -83.3774, state: 'GA', region: 'Southeast' },
  'auburn university': { lat: 32.6099, lng: -85.4808, state: 'AL', region: 'Southeast' },
  'louisiana state university': { lat: 30.4133, lng: -91.1800, state: 'LA', region: 'Southeast' },
  'university of florida': { lat: 29.6499, lng: -82.3486, state: 'FL', region: 'Southeast' },
  'ohio state university': { lat: 40.0067, lng: -83.0305, state: 'OH', region: 'Midwest' },
  'university of michigan': { lat: 42.2780, lng: -83.7382, state: 'MI', region: 'Midwest' },
  'penn state university': { lat: 40.7982, lng: -77.8599, state: 'PA', region: 'Northeast' },
};

export function findGeo(school: string) {
  ...
  return { lat: 39.5, lng: -98.35, state: 'US', region: 'Unknown' };
}
...
export default function ClientMapPage() {
  ...
  const jittered = chapters.map((c, _i) => {
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        ...
        <div className="xl:w-[65%] bg-[#141b27] rounded-2xl border border-slate-700/50 p-4 relative overflow-hidden">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="ml-auto text-xs text-slate-500">Click a pin for details</span>
          </div>
            ...
            <div className="absolute top-16 right-4 w-64 bg-[#0d1117] border border-slate-600 rounded-xl p-4 shadow-2xl z-50">
              ...
              <div className="flex items-center gap-2 mt-3">
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                  selected.status === 'active' || selected.status === 'won'
                    ? 'bg-emerald-900/50 text-emerald-400'
                    : 'bg-amber-900/50 text-amber-400'
                }`}>
                  {selected.status === 'active' ? 'Active' : selected.status === 'won' ? 'Won' : 'Onboarding'}
                </span>
        ...
        <div className="xl:w-[35%] flex flex-col gap-4">
          {/* ARR Progress */}
          <div className="bg-[#141b27] rounded-2xl border border-slate-700/50 p-4">
            <div className="flex items-center gap-2 mb-3">
              <DollarSign size={16} className="text-emerald-400" />
              <span className="text-sm font-semibold text-slate-300">ARR Progress</span>
            </div>
          ...
          <div className="bg-[#141b27] rounded-2xl border border-slate-700/50 p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-semibold text-slate-300">Chapter Growth Tiers</span>
            </div>
              ...
              {CHAPTER_TIERS.map((tier) => {
                ...
                return (
                  <div key={tier} className="flex items-center gap-2.5">
                    ...
                    <span className={`text-xs font-medium ${done ? 'text-emerald-400' : current ? 'text-amber-400' : 'text-slate-500'}`}>
                      {tier} chapters
                    </span>
          ...
          {regionData.length > 0 && (
            <div className="bg-[#141b27] rounded-2xl border border-slate-700/50 p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-semibold text-slate-300">Regional Breakdown</span>
              </div>
...
function StatCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="bg-[#141b27] rounded-xl border border-slate-700/50 p-3">
