 ...
 * Returns synced Linear projects for ticket creation and editing.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ data: (data ?? []) as LinearProjectOption[], error: null });
  } catch (err) {
    console.error('Unexpected error fetching linear projects:', err);
    return NextResponse.json(
