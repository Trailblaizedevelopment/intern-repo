...
import type { GmailMessage, ThreadMessage } from '../hooks/useGoogleIntegration';
...
interface InboxEmbedProps {
  google: GoogleIntegration;
  currentEmployee: Employee | null;
}

export function InboxEmbed({ google, currentEmployee }: InboxEmbedProps) {
  ...
  const { conversationThreads, newThreads, automatedThreads } = useMemo(() => {
    ...
    return { conversationThreads: conv, newThreads: newList, automatedThreads: auto };
  }, [threadsByThreadId, movedThreads, dismissedThreads]);

  const filterBySearch = useCallback((list: GmailMessage[]) => {
  ...
  if (!google.status?.connected) {
    return (
      <div className="ws-gmail-connect-container" style={{ padding: '32px 0' }}>
        <div className="ws-gmail-connect-card">
          ...
          <h2>Connect Gmail to see your inbox</h2>
  ...
  if (google.error) {
    return (
      <div style={{ padding: '24px', textAlign: 'center' }}>
        ...
        <p style={{ color: '#6B7280', marginBottom: 12 }}>{google.error}</p>
        <button className="ws-gmail-connect-main-btn" onClick={google.connect}>
          <Link2 size={16} /> Reconnect Gmail
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="ws-gmail-interface" style={{ minHeight: 500 }}>
      {/* Tabs */}
      <nav className="ws-inbox-tabs" role="tablist" aria-label="Inbox categories">
        <button type="button" role="tab" aria-selected={activeTab === 'conversations'}
        ...
        <button type="button" role="tab" aria-selected={activeTab === 'new'}
        ...
        <button type="button" role="tab" aria-selected={activeTab === 'automated'}
      ...
      <div className="ws-gmail-content">
        ...
        {activeTab === 'conversations' && selectedThreadId && (
          <div className="ws-gmail-detail ws-gmail-detail-chat">
            <div className="ws-gmail-detail-header">
              ...
              <span className="ws-gmail-detail-title">{selectedConversation?.subject || '(No subject)'}</span>
              <a href={`https://mail.google.com/mail/u/0/#inbox/${selectedThreadId}`} target="_blank"
                rel="noopener noreferrer" className="ws-gmail-detail-action" title="Open in Gmail">
            ...
            <div className="ws-gmail-chat-messages">
              {threadLoading ? (
                ...
                threadMessages.map(msg => (
                  <div key={msg.id} className={`ws-gmail-chat-bubble ${isOurEmail(msg.fromEmail) ? 'ours' : 'theirs'}`}>
            ...
            <div className="ws-gmail-detail-footer">
              <a href={`https://mail.google.com/mail/u/0/#inbox/${selectedThreadId}`} target="_blank"
                rel="noopener noreferrer" className="ws-gmail-reply-btn">
                <Reply size={16} /> Reply in Gmail
              </a>
            </div>
          </div>
        )}
        ...
        {activeTab === 'new' && selectedNewEmail && (
          <div className="ws-gmail-detail">
            <div className="ws-gmail-detail-header">
              <button className="ws-gmail-back-btn" onClick={() => setSelectedEmailId(null)}><X size={18} /></button>
              <a href={`https://mail.google.com/mail/u/0/#inbox/${selectedNewEmail.threadId}`} target="_blank"
                rel="noopener noreferrer" className="ws-gmail-detail-action" title="Open in Gmail">
                <ExternalLink size={16} />
              </a>
            </div>
            <div className="ws-gmail-detail-content">
              ...
              <div className="ws-gmail-detail-meta">
                <div className="ws-gmail-detail-avatar" style={{ backgroundColor: getAvatarColor(selectedNewEmail.fromEmail) }}>
              ...
              <div className="ws-gmail-detail-body">
                ...
                <a href={`https://mail.google.com/mail/u/0/#inbox/${selectedNewEmail.threadId}`} target="_blank"
                  rel="noopener noreferrer" className="ws-gmail-read-more">
                  Read full email in Gmail <ExternalLink size={14} />
                </a>
              </div>
              <div className="ws-gmail-detail-footer ws-gmail-detail-footer-actions">
                <button className="ws-gmail-move-btn"
                  onClick={() => { moveToConversations(selectedNewEmail.threadId); setSelectedEmailId(null); }}>
                  <ArrowRight size={16} /> Move to Conversations
                </button>
                <button className="ws-gmail-delete-btn" onClick={() => dismissFromNew(selectedNewEmail.threadId)}>
        ...
        {activeTab === 'automated' && selectedAutomatedEmail && (
          <div className="ws-gmail-detail">
            <div className="ws-gmail-detail-header">
              <button className="ws-gmail-back-btn" onClick={() => setSelectedEmailId(null)}><X size={18} /></button>
              <a href={`https://mail.google.com/mail/u/0/#inbox/${selectedAutomatedEmail.threadId}`} target="_blank"
                rel="noopener noreferrer" className="ws-gmail-detail-action" title="Open in Gmail">
                <ExternalLink size={16} />
              </a>
            </div>
            <div className="ws-gmail-detail-content">
              ...
              <div className="ws-gmail-detail-meta">
                <div className="ws-gmail-detail-avatar ws-gmail-detail-avatar-auto"><Bot size={18} /></div>
                <div className="ws-gmail-detail-sender">
